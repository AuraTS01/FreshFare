...
// AJAX request for item suggestions

if(isset($_GET['ajax_item_suggest']) && $_GET['ajax_item_suggest']==1 && !empty($_GET['term'])) {
    $term = strtolower(trim($_GET['term']));
    $suggestions = [];

    // Fetch items from company_registration & item_price
    $sql = "SELECT cr.company_name, cr.selling_items
            FROM company_registration cr
            JOIN item_price ip ON cr.company_id = ip.company_id";
    $res = mysqli_query($login_db, $sql);
    if($res && mysqli_num_rows($res) > 0){
        while($row = mysqli_fetch_assoc($res)){
            $items = explode(',', $row['selling_items']);
            foreach($items as $item){
                $item = trim($item);
                if(stripos($item, $term)!==false){
                    $suggestions[] = $item . " (" . $row['company_name'] . ")";
                }
            }
        }
    }
    $suggestions = array_values(array_unique($suggestions)); // remove duplicates
                    ...
                    <div class="hero__search">
                        <div class="hero__search__form">
                            <form id="searchForm" action="javascript:void(0);">
                                <input type="text" id="searchInput" placeholder="What do you need?" class="form-control">
    ...
    <!-- Featured Section Begin -->
       ...
       <?php
        ...
        $sql = "SELECT cr.company_id, cr.company_name, cr.email, cr.selling_items,
                ...
                FROM company_registration cr
                JOIN item_price ip ON cr.company_id = ip.company_id";
        $com_result = mysqli_query($login_db, $sql);
        ?>
        ...
        <section id="targetSection" class="featured spad">
            <div class="container">
                ...
                <div class="row featured__filter">
                    <?php if ($com_result && mysqli_num_rows($com_result) > 0): ?>
                        <?php while ($row = mysqli_fetch_assoc($com_result)): ?>
                            <?php
                            ...
                            <!-- Company Modal -->
                            <div class="modal fade companyModal" 
                                            id="companyModal<?php echo $companyId; ?>" 
                                data-items='<?php echo json_encode(array_column($uniqueItems, "name")); ?>' 
                                tabindex="-1">
